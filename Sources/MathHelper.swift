...
class MathHelper {
    public static func calculateMaxDiff(actual: Array<Array<[Double]>>, expected: Array<Array<[Double]>>) -> Double {
        var diff = Array(repeating: Array(repeating: Array(repeating: 0.0, count: Settings.M), count: Settings.K+2), count: Settings.N)
        
        var max = 0.0
        
        for n in 0...Settings.N-1 {
    ...
    public static func calculateMaxDiff1(actual: Array<Array<[Double]>>, expected: Array<Array<[Double]>>) -> Double {
        let diff1 = expected[Settings.N-1][1][Int(Double(Settings.M-1) / 2)]
        let diff2 = actual[Settings.N-1][1][Int(Double(Settings.M-1) / 2)]
        print("expected = \(diff1), actual = \(diff2)")
        return abs(diff1 - diff2)
    }

    public static func calculateExactFunction() -> Array<Array<[Double]>> {
        var result = Array(repeating: Array(repeating: Array(repeating: 0.0, count: Settings.M), count: Settings.K+2), count: Settings.N)
